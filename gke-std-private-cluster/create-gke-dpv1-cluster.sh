...
# - Dataplane V1 (Legacy CNI / standard kube-proxy networking)
...
display_configuration() {
    ...
    echo "  Dataplane V1:       ✓ Enabled (Legacy CNI / standard kube-proxy)"
    echo ""
...
create_cluster() {
    ...
    gcloud beta container clusters create "$CLUSTER_NAME" \
        ...
        \
        `# Dataplane V1 (Legacy CNI - omit dataplane-v2 flags to use LEGACY_DATAPATH)` \
        \
...
verify_cluster() {
    ...
    # Verify Dataplane V1
    log_info "Verifying Dataplane V1..."
    DATAPLANE=$(gcloud container clusters describe "$CLUSTER_NAME" \
        --region="$REGION" \
        --project="$PROJECT_ID" \
        --format="value(networkConfig.datapathProvider)")

    if [ "$DATAPLANE" == "LEGACY_DATAPATH" ]; then
        log_success "Dataplane V1 (LEGACY_DATAPATH) is enabled"
    else
        log_warning "Dataplane provider status: $DATAPLANE (expected LEGACY_DATAPATH)"
    fi
...
main() {
    echo ""
    log_info "Starting GKE Private Cluster Deployment (Dataplane V1)"
    echo ""
